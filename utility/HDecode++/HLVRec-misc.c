...
void CheckTokenSetOrder (DecoderInst *dec, TokenSet *ts)
{
   ...
   if (!ok) {
      fprintf (LOG_STREAM, "XXXXX CheckTokenSetOrder \n");
      PrintTokSet (dec, ts);
      abort();
   }
}
...
static void CheckTokenSetId (DecoderInst *dec, TokenSet *ts1, TokenSet *ts2)
{
   ...
   if (!ok) {
      fprintf (LOG_STREAM, "XXXXX CheckTokenSetId  difference in tokensets \n");
      PrintTokSet (dec, ts1);
      PrintTokSet (dec, ts2);
      abort();
   }
...
void Debug_DumpNet (LexNet *net)
{
   ...
   if (!debugFile) {
      fprintf (LOG_STREAM, "fopen failed\n");
      return;
   }
   ...
   for (l = 0; l < dec->nLayers; ++l) {
      ...
      for (inst = dec->instsLayer[l]; inst; inst = inst->next) {
         ...
         if (inst->best < dec->beamLimit)
            fprintf (LOG_STREAM, "layer %d (LexNodeInst *) %p\n", l, inst);
#endif
         ln = inst->node;
         N = (ln->type == LN_MODEL) ? ln->data.hmm->numStates : 1;
         for (j = 0; j < N; ++j) {
            ...
            if (ts->n > 0) {
               ...
               for (k = 0; k < ts->n; ++k) {
                  ...
                  if (ts->score + rtok->delta < dec->beamLimit) {
                     fprintf (LOG_STREAM, "l %d *((LexNodeInst *) %p) *((TokenSet *) %p)\n", l, inst, ts);
                  }
               }
#endif
            }
      ...
      fprintf (LOG_STREAM, "l %d aveTok/TS %.3f maxTok %d\n",
...
void InitPhonePost (DecoderInst *dec)
{
   ...
   do {
      ...
      if (!phoneId->aux) {
         ++dec->nPhone;
         phoneId->aux = (Ptr) (intptr_t) dec->nPhone;
   ...
   fprintf (LOG_STREAM, "found %d monophones\n", dec->nPhone);
...
#if 0 
         fprintf (LOG_STREAM, "phone %d freq %d post %f\n", i, phoneFreq[i],
                 phonePost[i]);
#endif
      } else 
         phonePost[i] = LZERO;
...
void AccumulateStats (DecoderInst *dec)
{
   ...
   /* count inst/ts/tok, find best/worst scores */
   {
      ...
      for (l = 0; l < dec->nLayers; ++l) {
         ...
         fprintf (LOG_STREAM, "STATS layer %d  ", l);
         fprintf (LOG_STREAM, "%5d Insts   %5d TokenSets   %6d Tokens  %.2f Tok/TS ",
                 ls->nInst, ls->nTS, ls->nTok, (float) ls->nTok / ls->nTS);
         fprintf (LOG_STREAM, "best:  %.4f   ", ls->bestScore);
         fprintf (LOG_STREAM, "worst: %.4f\n", ls->worstScore);
