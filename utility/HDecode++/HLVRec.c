...
#if 0
   fprintf (LOG_STREAM, "sizeof(Token) = %d\n", sizeof (Token));
   fprintf (LOG_STREAM, "sizeof(LexNodeInst) = %d\n", sizeof (LexNodeInst));
   fprintf (LOG_STREAM, "sizeof(WordendHyp) = %d\n\n", sizeof (WordendHyp));
#endif
...
{       
   ...
   */
   ...
   if (trace & T_MEM) {
      fprintf (LOG_STREAM, "memory stats at start of recognition\n");
      PrintAllHeapStats ();
   }
...
#ifdef DEBUG_TRACE
   if (trace & T_ACTIV)
      fprintf (LOG_STREAM, "allocating %d tokens in array for node in layer %d\n", ((l == LAYER_SIL) ? LAYER_SIL_NTOK_SCALE : 1) * dec->nTok, l);
#endif
...
#ifdef DEBUG_TRACE
   if (trace & T_ACTIV)
      fprintf (LOG_STREAM, "activated node in layer %d\n", l);
#endif
