...
/*
         if (!dec->si->useHModel) 
            ...
            OutPBlock_HMod (dec->si, &dec->obsBlock[0], cache->block,
                            ...
                            dec->frame);
*/
         cache->stateOutP[sIdx * cache->block] = 0.0;
         if(dec->gmm_weight > 0.0) {
             if(dec->score_interface_gmm_mllr_ptr != NULL) {
                 float gmm_mllr_score = dec->score_interface_gmm_mllr_ptr->score(dec->frame-1, sIdx);
                 cache->stateOutP[sIdx * cache->block] += gmm_mllr_score;
             }
             else if(dec->score_interface_gmm_ptr != NULL) {
                 float gmm_score = dec->score_interface_gmm_ptr->score(dec->frame-1, sIdx);
                 cache->stateOutP[sIdx * cache->block] += ( dec->gmm_weight * gmm_score );
             }
             else {}
         }
/*
         if((dec->gmm_weight > 0.0)&&(dec->score_interface_gmm_ptr != NULL)) {
             float gmm_score = dec->score_interface_gmm_ptr->score(dec->frame-1, sIdx);
             cache->stateOutP[sIdx * cache->block] += ( dec->gmm_weight * gmm_score );
         }
*/
         if((dec->sgmm_weight > 0.0)&&(dec->score_interface_sgmm_ptr != NULL)) {
             float sgmm_score = dec->score_interface_sgmm_ptr->score(dec->frame-1, sIdx);
             cache->stateOutP[sIdx * cache->block] += ( dec->sgmm_weight * sgmm_score );
         }
         if((dec->sgmm2_weight > 0.0)&&(dec->score_interface_sgmm2_ptr != NULL)) {
             float sgmm2_score = dec->score_interface_sgmm2_ptr->score(dec->frame-1, sIdx);
             cache->stateOutP[sIdx * cache->block] += ( dec->sgmm2_weight * sgmm2_score );
         }
         if((dec->score_weight > 0.0)&&(dec->senone_score_table != NULL)) {
             const kaldi::Matrix<BaseFloat>& table = dec->senone_score_table->Value();
             assert(sIdx < dec->senone_map_score_ptr->size());
             int32 pdf = (*dec->senone_map_score_ptr)[sIdx];
             float table_score = table(dec->frame-1, pdf);
             cache->stateOutP[sIdx * cache->block] += ( dec->score_weight * table_score);
         }
         cache->stateOutP[sIdx * cache->block] *= dec->acScale;
