...
// vulcan header files
#include "am-phoneset.h"

using std::vector;
using kaldi::int32;
using kaldi::BaseFloat;
using kaldi::RandomAccessBaseFloatVectorReaderMapped;
using kaldi::RandomAccessInt32VectorVectorReader;
using kaldi::SequentialBaseFloatMatrixReader;
using kaldi::TransitionModel;
using kaldi::ContextDependency;
using kaldi::AmDiagGmm;
using kaldi::DecodableAmDiagGmm;
using kaldi::DecodableAmDiagGmmRegtreeMllr;
using kaldi::RegressionTree;
using kaldi::RandomAccessRegtreeMllrDiagGmmReaderMapped;
using kaldi::RegtreeMllrDiagGmm;
using kaldi::DecodableAmDiagGmmRegtreeMllr;
using kaldi::AmSgmm;
using kaldi::SgmmGselectConfig;
using kaldi::SgmmPerSpkDerivedVars;
using kaldi::DecodableAmSgmm;
using kaldi::AmSgmm2;
using kaldi::Sgmm2PerSpkDerivedVars;
using kaldi::DecodableAmSgmm2;
using vulcan::PhoneSet;

class SenoneScoreAmDiagGmm : public DecodableAmDiagGmm {
    public:
    SenoneScoreAmDiagGmm(const AmDiagGmm &am,
                         const TransitionModel &tm,
                         const kaldi::Matrix<BaseFloat> &feats,
                         const vector<int32>& senone_index_map,
                         BaseFloat log_prune)
        : DecodableAmDiagGmm(am, tm, feats, log_prune),
          _senone_index_map(senone_index_map) {}
    BaseFloat score(int32 frame, int sIdx) {
        assert(sIdx < _senone_index_map.size());
        int32 pdf_id = _senone_index_map[sIdx];
        return LogLikelihoodZeroBased(frame, pdf_id);
    }
    private:
    const vector<int32>& _senone_index_map;
    KALDI_DISALLOW_COPY_AND_ASSIGN(SenoneScoreAmDiagGmm);
};

class SenoneScoreAmDiagGmmRegtreeMllr : public DecodableAmDiagGmmRegtreeMllr {
    public:
    SenoneScoreAmDiagGmmRegtreeMllr(const AmDiagGmm &am,
                                    const TransitionModel &tm,
                                    const kaldi::Matrix<BaseFloat> &feats,
        ...
        : DecodableAmDiagGmmRegtreeMllr(am, tm, feats, mllr_xform, regtree, scale, log_sum_exp_prune),
          _senone_index_map(senone_index_map) {}
    BaseFloat score(int32 frame, int sIdx) {
        assert(sIdx < _senone_index_map.size());
        int32 pdf_id = _senone_index_map[sIdx];
    ...
    private:
    const std::vector<int32>& _senone_index_map;
    KALDI_DISALLOW_COPY_AND_ASSIGN(SenoneScoreAmDiagGmmRegtreeMllr);
};

class SenoneScoreAmSgmm : public DecodableAmSgmm {
    public:
    SenoneScoreAmSgmm(const SgmmGselectConfig &opts,
                      const AmSgmm &am,
                      ...
                      const vector<int32>& senone_index_map,
                      BaseFloat log_prune)
        : DecodableAmSgmm(opts, am, spk, tm, feats, gselect, log_prune),
          _senone_index_map(senone_index_map) {}
    BaseFloat score(int32 frame, int sIdx) {
        assert(sIdx < _senone_index_map.size());
        int32 pdf_id = _senone_index_map[sIdx];
    ...
    private:
    const vector<int32>& _senone_index_map;
    KALDI_DISALLOW_COPY_AND_ASSIGN(SenoneScoreAmSgmm);
};

class SenoneScoreAmSgmm2 : public DecodableAmSgmm2 {
    public:
    SenoneScoreAmSgmm2(const AmSgmm2 &am,
                       const TransitionModel &tm,
                       const kaldi::Matrix<BaseFloat> &feats,
                       const vector<vector<int32> > &gselect,
                       const vector<int32>& senone_index_map, 
        ...
        : DecodableAmSgmm2(am, tm, feats, gselect, log_prune, spk) , 
          _senone_index_map(senone_index_map) {}
    BaseFloat score(int32 frame, int sIdx) {
        assert(sIdx < _senone_index_map.size());
        int32 pdf_id = _senone_index_map[sIdx];
        return LogLikelihoodForPdf(frame, pdf_id);
    }
    private:
    const vector<int32>& _senone_index_map;
    KALDI_DISALLOW_COPY_AND_ASSIGN(SenoneScoreAmSgmm2);
};
...
struct _DecoderInst {
   /* extra data members */
   PhoneSet* phone_set_ptr;
   /* GMM data members */
   TransitionModel* trans_gmm_ptr;
   AmDiagGmm* am_gmm_ptr;
   ContextDependency* tree_gmm_ptr;
   vector<int32>* senone_map_gmm_ptr;
   /* MLLR data members */
   RegressionTree* mllr_regtree;
   RandomAccessRegtreeMllrDiagGmmReaderMapped* mllr_reader;
   /* Subspace GMM data members */
   TransitionModel* trans_sgmm_ptr;
   AmSgmm* am_sgmm_ptr;
   RandomAccessBaseFloatVectorReaderMapped* spkvecs_reader_sgmm_ptr;
   SgmmPerSpkDerivedVars* spk_vars_sgmm_ptr;
   RandomAccessInt32VectorVectorReader* gselect_reader_sgmm_ptr;
   ContextDependency* tree_sgmm_ptr;
   vector<int32>* senone_map_sgmm_ptr;
   /* Subspace GMM 2.0 data members */
   TransitionModel* trans_sgmm2_ptr;
   AmSgmm2* am_sgmm2_ptr;
   RandomAccessBaseFloatVectorReaderMapped* spkvecs_reader_sgmm2_ptr;
   Sgmm2PerSpkDerivedVars* spk_vars_sgmm2_ptr;
   RandomAccessInt32VectorVectorReader* gselect_reader_sgmm2_ptr;
   ContextDependency* tree_sgmm2_ptr;
   vector<int32>* senone_map_sgmm2_ptr;
   /* Senone score table data members */
   ContextDependency* tree_score_ptr;
   vector<int32>* senone_map_score_ptr;
   /* GMM senone score interface */
   SenoneScoreAmDiagGmm* score_interface_gmm_ptr;
   /* GMM with MLLR adaptation score interface */
   SenoneScoreAmDiagGmmRegtreeMllr* score_interface_gmm_mllr_ptr;
   /* Subspace GMM senone score interface */
   SenoneScoreAmSgmm* score_interface_sgmm_ptr;
   /* Subspace GMM 2.0 senone score interface */
   SenoneScoreAmSgmm2* score_interface_sgmm2_ptr;
   /* Reader for acoustic feature and senone score table */
   SequentialBaseFloatMatrixReader* acoustic_feature;
   SequentialBaseFloatMatrixReader* senone_score_table;
   /* Parameters */
   BaseFloat log_prune;
   float gmm_weight;
   float sgmm_weight;
