...
StateInfo_lv *ConvertHSet(MemHeap *heap, HMMSet *hset, Boolean useHModel)
{
   ...
   while (GoNextState (&hss, FALSE)) {
      if (hss.si->sIdx == -1) {
         ...
         fprintf (LOG_STREAM, "THIS SHOULD NOT HAPPEN!!!\n");
      }
   }
   EndHMMScan (&hss);
...
void PrintState_lv (StateInfo_lv *si,  unsigned short s)
{
   ...
   for (m = 1; m <= nMix; m++) {
      ...
      fprintf (LOG_STREAM, "mix %d  mixw %.2f gc %.2f  \n",m, mixw, gc);

         
      fprintf (LOG_STREAM, " mean ");
      for (i = 0; i < si->nDim; ++i) {
         fprintf (LOG_STREAM, "%.2f ", mean[i]);
      }
      fprintf (LOG_STREAM, "\n invVar ");
      for (i = 0; i < si->nDim; ++i) {
         fprintf (LOG_STREAM, "%.2f ", invVar[i]);
      }
      fprintf (LOG_STREAM, "\n");
      base += si->floatsPerMix;
      mean += si->floatsPerMix;
