...
static LogFloat GetProb(Source *src, Boolean bin)
{
   ...
   if (LN10*prob < -90.0)
      fprintf (LOG_STREAM, "REALLY small: %f\n", LN10*prob);
   else {
...
static LMId UnigramLMIdMapper(FSLM_ngram *nglm, char *w)
{
   ...
   if (labId) {
      ...
      if (!word) {
         ...
         labId->aux = (Ptr) (intptr_t) (int) idCounter;
         id = idCounter;
      }
   }
   else {       /* label unknown */
                           ...
                           Vocab *vocab)
{
   ...
   LMId ndx[NSIZE+1];
   NEntry *ne, *le = NULL;
   SEntry *tmpSE, *curtmpSE; curtmpSE = NULL;
   int ntmpSE = 0;
   LMId (*word2lmid)(FSLM_ngram *, char *);
   Word word;
   Pron pron;
      ...
      if (n == 1) {
         ...
         if (word) {
            for (pron = word->pron; pron; pron = pron->next) {
               pronid = (PronId) (intptr_t) pron->aux;
...
void SetStartEnd (FSLM *lm, char *startWord, char *endWord, Vocab *vocab)
{
   ...
   lm->startPronId = (PronId) (intptr_t) word->pron->aux;
   ...
   lm->endPronId = (LMId) (intptr_t) word->pron->aux;
}
   ...
   
   {
      ...
      for (i = 0; i < lat->nn; ++i) {
         ...
         for (la = ln->foll; la; la = la->farc) {
            for (pron = la->end->word->pron; pron; pron = pron->next) {
               if (pron->aux > 0) {
                  fslmla[p].word = (PronId) (intptr_t) pron->aux;
...
LogFloat LMTransProb_ngram (FSLM *lm, LMState src, PronId pronid, LMState *dest)
{
   ...
   if (trace & T_ACCESS)
      fprintf (LOG_STREAM, "src %p PronId %u\n", src, (unsigned int) pronid);
   ...
   if (trace & T_ACCESS)
      fprintf (LOG_STREAM, "lmprob = %f  dest %p\n", NGLM_PROB_TO_FLOAT(lmprob), *dest);
...
{
   NEntry *ne_tg, *ne_bg;
   SEntry *se_tg, *seLast_tg, *se_bg, *seLast_bg; seLast_tg = seLast_bg = NULL;
   PronId p, pend;
   NGLM_Prob *unigrams;
   NGLM_Prob maxScore = NGLM_PROB_LZERO;
...
void PrintLMHashStats(FSLM *lm)
{
   ...
   for (hash = 0; hash < nglm->hashsize; hash++) {
      ...
      fprintf (LOG_STREAM, "bin %d has %d entries\n", hash, n);
   }
}
