...
#define MARK_ALTPATH(a)         (a->prev = (WordendHyp*) ((intptr_t)(a->prev) | MARK_ALTPATH_MASK))
#define MARKED_ALTPATH_P(a)     ((intptr_t)((a)->prev) & MARK_ALTPATH_MASK)
#define UNMARK_ALTPATH(a)       (a->prev = (WordendHyp*) ((intptr_t)(a->prev) & ~MARK_ALTPATH_MASK))

#define GC_ALTPATH_PREV(a)      ((WordendHyp *) ((intptr_t)(a)->prev & ~MARK_ALTPATH_MASK))

#ifdef MODALIGN
#define MARK_MODPATH_MASK     0x00000001UL
#define MARK_MODPATH(m)         (m->ln = (LexNode*) ((intptr_t)((m)->ln) | MARK_MODPATH_MASK))
#define MARKED_MODPATH_P(m)     ((intptr_t)((m)->ln) & MARK_MODPATH_MASK)
#define UNMARK_MODPATH(m)       (m->ln = (LexNode*) ((intptr_t)((m)->ln) & ~MARK_MODPATH_MASK))
...
static void SweepPaths (MemHeap *heap)
{
   ...
   for (b = heap->heap; b; b = b->next) {
      ...
      if (b->numFree == b->numElem) { 
         if (trace&T_GC)
            fprintf (LOG_STREAM, "should free whole block!\n");
      }
   }

   if (trace&T_GC)
      fprintf (LOG_STREAM, "freed %d of %d Paths\n", freed, total);
}
...
static void SweepAltPaths (MemHeap *heap)
{
   ...
   if (trace&T_GC)
      fprintf (LOG_STREAM, "freed %d of %d AltPaths\n", freed, total);
}
...
static void SweepModPaths (MemHeap *heap)
{
   ...
   if (trace&T_GC)
      fprintf (LOG_STREAM, "freed %d of %d ModPaths\n", freed, total);
}
#endif
...
static void GarbageCollectPaths (DecoderInst *dec)
{
   ...
   if (trace&T_GC) {
      fprintf (LOG_STREAM, "Garbage Collecting paths.\n");
   ...
   if (trace&T_GC) {
      fprintf (LOG_STREAM, "Garbage Collection finished.\n");
      PrintHeapStats (&dec->weHypHeap);
      PrintHeapStats (&dec->altweHypHeap);
#ifdef MODALIGN
   if (dec->modAlign)
