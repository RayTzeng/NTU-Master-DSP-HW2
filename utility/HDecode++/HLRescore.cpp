...
static char *startWord = "<s>";         /* word at start of Lattice (!SENT_START) */
static LabId startLab;          /* corresponding LabId */
static char *endWord = "</s>";           /* word at end of Lattice (!SENT_END) */
...
static Boolean fixBadLats = TRUE;         /* fix final word in lattices */
...
static Boolean findBest = TRUE;    /* -f */
...
void ReportUsage(void)
{
   ...
   fprintf(LOG_STREAM, " Option                                   Default\n\n");
   fprintf(LOG_STREAM, " -i s    Output transcriptions to MLF s       off\n"); 
   fprintf(LOG_STREAM, " -l s    dir to store label/lattice files     current\n");
   fprintf(LOG_STREAM, " -m s    merge nodes and arcs of lattice      off\n");
   fprintf(LOG_STREAM, " -n s    load n-gram LM and expand lattice    off\n");
   fprintf(LOG_STREAM, " -wn s   load word pair LM network            off\n");
   fprintf(LOG_STREAM, " -o s    output label formating NCSTWMX       none\n");
   fprintf(LOG_STREAM, " -t f [f] input lattice pruning threshold     off\n");
   fprintf(LOG_STREAM, " -u f [f] output lattice pruning threshold    off\n");
   fprintf(LOG_STREAM, " -p f    inter model trans penalty (log)      0.0\n");
   fprintf(LOG_STREAM, " -s f    grammar scale factor                 1.0\n");
   fprintf(LOG_STREAM, " -a f    acoustic scale factor                1.0\n");
   fprintf(LOG_STREAM, " -r f    pronunciation scale factor           1.0\n");
   fprintf(LOG_STREAM, " -d      get pronprobs from dict              off\n");
   fprintf(LOG_STREAM, " -c      calculate statistics                 off\n");
   fprintf(LOG_STREAM, " -f      find 1-best transcription            off\n");
   fprintf(LOG_STREAM, " -w      write output lattices                off\n");
   fprintf(LOG_STREAM, " -q s    output lattice format                tvaldmnr\n"); 
   fprintf(LOG_STREAM, " -y s    output label file extension          rec\n");
...
int main(int argc, char *argv[])
{
   ...
   const char *usage =
        "Rescore lattices with different acoustic / language model weights\n"
        "Usage:  Hybrid.HLRescore [options]\n";
   const char *program = "Hybrid.HLRescore";
   ...
   try {
       ...
       po.Register("lm-weight", &lmScale,
                   "scaling factor for n-gram probabilities");
       string lat_dir = "";
       po.Register("lat-dir", &lat_dir,
                   "directory where lattices were stored");
       string lex = "";
       po.Register("lex", &lex, 
                   "lexicon");
       string list = "";
       po.Register("list", &list,
                   "utterance list");
       string mlf = "";
       po.Register("mlf", &mlf,
                   "result in HTK format");
       po.Read(argc, argv);
       if(argc < 2) {
           po.PrintUsage();
           return 1;
...
//       po.PrintConfig(std::cerr);
       ...
       if(lat_dir != "") {
           latInDir = (char*) malloc((lat_dir.length()+1)*sizeof(char));
           strcpy(latInDir, lat_dir.c_str());
       }
       if(lex != "") {
       ...
       vulcan::Stream stream;
       stream.open(list, "r");
       FILE* f = stream.ptr();
       latfn = (char*) malloc(LINE_BUFFER*sizeof(char));
       size_t n = 0;
       while (1) {
           fgets(latfn, LINE_BUFFER, f);
           if(feof(f)) { break; }
           latfn[strlen(latfn)-1] = '\0';
           n++;
           if (trace & T_TOP) {
              fprintf (LOG_STREAM, "File [ %05zu ] : [ %s ]\n", n, latfn);
           }
...
*/
   ...
   if (lmFile) {
      if (trace & T_TOP) 
         fprintf (LOG_STREAM, "Reading LM from %s\n", lmFile);
   ...
   if (wpNetFile) {
      if (trace & T_TOP) 
         fprintf (LOG_STREAM, "Reading word pair LM from %s\n", wpNetFile);
   ...
   if (trace & T_MEM) {
      fprintf(LOG_STREAM, "Memory State on Completion\n");
      PrintAllHeapStats();
   }
...
void ProcessLattice (char *latfn)
{
   ...
   if (trace & T_LAT)
      fprintf (LOG_STREAM, "lattice size: %d nodes/ %d arcs\n", lat->nn, lat->na);
   ...
   if (trace & T_MEM) {
      fprintf(LOG_STREAM, "Memory State after processing lattice\n");
      PrintAllHeapStats();
   }
   ResetHeap (&latHeap);
}
...
void ProcessLabels (char *labfn)
{
   ...
   if (trace&T_TOP) {
      fprintf(LOG_STREAM, "Created lattice with %d nodes / %d arcs from label file\n",
             lat->nn, lat->na);
      fflush(stdout);
   }
...
void ApplyNGram2LabLat(Lattice *lat, LModel *lm)
{
   ...
   if (trace & T_EXP) {
      ...
      fprintf(stdout, "\n Vocab entries: ");
      for (j = 0; j < lat->nn - 2; j++) {
         fprintf(stdout, "%s ", lm->data.ngram->wdlist[(intptr_t) revlab[j]->aux]->name);
      }
      fprintf(stdout, "\n\n");
      fflush(stdout);
   }
