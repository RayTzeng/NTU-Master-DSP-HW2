...
static void PrintPath (DecoderInst *dec, WordendHyp *we)
{
   ...
   for (; we; we = we->prev) {
      ...
      fprintf (LOG_STREAM, "%s%d (%d %.3f %.3f)  ", pron->word->wordName->name, pron->pnum, 
...
{
   fprintf (LOG_STREAM, "lmState %p score %f path %p ", tok->lmState, tok->score, tok->path);
...
{
   fprintf (LOG_STREAM, "lmState %p delta %f path %p ", tok->lmState, tok->delta, tok->path);
   PrintPath (dec, tok->path);
   fprintf (LOG_STREAM, "\n");
}
...
static void PrintTokSet (DecoderInst *dec, TokenSet *ts)
{
   ...
   fprintf (LOG_STREAM, "n = %d  score = %f id = %d\n", ts->n, ts->score, ts->id);
...
Transcription *TraceBack(MemHeap *heap, DecoderInst *dec)
{
   ...
   if (trace & T_TOP) {
      fprintf (LOG_STREAM, "best score %f\n", ts->score);
                       ...
                       int *na)
{
   ...
   if (!lat->lnodes[n].hook) {      /* not seen yet */
      ...
      if (trace & T_LAT)
         fprintf (LOG_STREAM, "J=%d S=%d E=%d a=%f l=%f\n", *na, s, n, la->aclike, la->lmlike);
...
Lattice *LatTraceBack (MemHeap *heap, DecoderInst *dec)
{
   ...
   WordendHyp *sentEndWE = NULL;

   if (!dec->net->end->inst)
      HError (-9999, "LatTraceBack: end node not active");
   else
//      fprintf (LOG_STREAM, "found %d tokens in end state\n", dec->net->end->inst->ts->n);
...
//   fprintf (LOG_STREAM, "nnodes %d nlinks %d\n", nnodes, nlinks);
...
void PrintModPath (DecoderInst *dec, ModendHyp *m)
{
   ...
   for (; m; m = m->prev) {
      ...
      fprintf (LOG_STREAM, "(%d %s %s) ", m->frame, t, s);
   }
   fprintf (LOG_STREAM, "\n");
}
...
//         fprintf (LOG_STREAM, "CheckLAlign: MODALIGN Sanity check failed! %d laDur %.2f  dur %.2f\n", i, laDur, dur);
   }
}
#endif
...
//   fprintf (LOG_STREAM, "found %d arcs\n", i);
   return path;
}
