                         ...
                         LogFloat score, Boolean prune)
{
   ...
   if (dest->n == 0) {   /* empty dest tokenset -> just copy src */
      ...
      /*       fprintf (LOG_STREAM, "MergeTokSet: pruned src TS\n"); */
      return;
   }
#ifdef TSIDOPT
      ...
      if (nWinTok <= dec->nTok) {
         ...
         /*          fprintf (LOG_STREAM, "MTS: %d nWin  %d src  %d dest\n", nWinTok, nWin[0], nWin[1]); */
         if (nWin[0] == nWinTok)
...
#define NBINS 64
         ...
         if (nTok == dec->nTok) {
            ...
            /*             fprintf (LOG_STREAM, "nBetter %d\n", nBetter); */

            if (nBetter > dec->nTok) {  /* multiple tokens with delta == limit
                                           ==> delete some */
               for (i = 0; i < nWinTok; ++i) {
                  if(nBetter <= dec->nTok) { break; }
                  if (winTok[i].delta == limit) {
...
static void PropagateInternal (DecoderInst *dec, LexNodeInst *inst)
{
   ...
   if (hmm->tIdx < 0) {
      ...
      instTS[N-2].score += (dec->loop_scale * trP[N-1][N-1]);
      ...
      for (i = N-2; i >=2; --i) {             /* for all internal states, except N-1 */
         ...
         if (ts->n > 0) {   /*  && (trP[i][i+1] > LSMALL)) { */
            ...
            MergeTokSet (dec, ts, ts+1 , (dec->transition_scale * trP[i][i+1]), (!mergeTokOnly));
            
            /* loop transition */
            ts->score += (dec->loop_scale * trP[i][i]);
         }
      }
      
      /* entry transition i=1 -> j=2 */
      if ((instTS[0].n > 0) && (trP[1][2] > LSMALL)) {
         MergeTokSet (dec, &instTS[0], &instTS[1], (dec->transition_scale * trP[1][2]), (!mergeTokOnly));
      }
      ...
      if (instTS[N-2].n > 0) {      /* && (trP[N-1][N] > LSMALL)) { */
         ...
         MergeTokSet (dec, &instTS[N-2], &instTS[N-1], (dec->transition_scale * trP[N-1][N]), FALSE);
      }
   ...
   /* general (not left-to-right) PropagateInternal   */
   {
...
#ifdef DEBUG_TRACE
      if (trace & T_PROP)
         fprintf (LOG_STREAM, "#########################PropagateInternal hmm %p '%s':\n", inst->node,
...
#ifdef DEBUG_TRACE
            if (trace & T_PROP)
               fprintf (LOG_STREAM, "transP[%d][%d] = %f\n", i, j, trP[i][j]);
#endif
            if ((instTS[i-1].n > 0) && (trP[i][j] > LSMALL)) {
               if(i == j) {
                   MergeTokSet (dec, &instTS[i-1], &tempTS[j-1], (dec->loop_scale * trP[i][j]), (!mergeTokOnly));
               }
               else {
                   MergeTokSet (dec, &instTS[i-1], &tempTS[j-1], (dec->transition_scale * trP[i][j]), (!mergeTokOnly));
               }
            }
         }
         if (tempTS[j-1].n > 0) {
...
#ifdef DEBUG_TRACE
         if (trace & T_PROP) {
            fprintf (LOG_STREAM, "PropagateInternal: tokens in state %d after OutP: ", j);
      ...
      for (i = 2; i < N; ++i) {
         ...
         if ((instTS[i-1].n > 0) && (trP[i][j] > LSMALL)) {
            if(i == j) {
                MergeTokSet (dec, &instTS[i-1], &instTS[j-1], (dec->loop_scale * trP[i][j]), FALSE);
            }
            else {
                MergeTokSet (dec, &instTS[i-1], &instTS[j-1], (dec->transition_scale * trP[i][j]), FALSE);
            }
         }
      }
...
#ifdef DEBUG_TRACE
      if (trace & T_PROP) {
         fprintf (LOG_STREAM, "PropagateInternal: tokens in exit state %d: ", j);
         PrintTokSet (dec, &instTS[j-1]);
         fprintf (LOG_STREAM, "-------------------\n");
      }
#endif
...
#ifdef DEBUG_TRACE
      if (trace & T_PROP) {
         fprintf (LOG_STREAM, "best %f\n", inst->best);
         for (i = 0; i < N; ++i)
            fprintf (LOG_STREAM, " %d ",inst->ts[i].n);
...
#ifdef DEBUG_TRACE
         if (trace & T_PROP) {
            fprintf (LOG_STREAM, "found tee HMM node '%s' transp %f\n",
                    FindMacroStruct (dec->net->hset, 'h', hmm)->id->name,
                    hmm->transP[1][N]);
            fprintf (LOG_STREAM, "PropagateExternal: entry state:\n");
            PrintTokSet (dec, entryTS);
            fprintf (LOG_STREAM, "----\n");
            fprintf (LOG_STREAM, "PropagateExternal: exit state:\n");
...
#ifdef DEBUG_TRACE
         if (trace & T_PROP) {
            fprintf (LOG_STREAM, "PropagateExternal: exit state after tee propagation:\n");
            PrintTokSet (dec, exitTS);
            fprintf (LOG_STREAM, "----\n");
         }
#endif
...
static void HandleWordend (DecoderInst *dec, LexNode *ln)
{
   ...
   if (trace & T_WORD) {
      fprintf (LOG_STREAM, "PropagateWordEnd: handleWordend '%s'\n", dec->net->pronlist[ln->data.pron]->word->wordName->name);
      fprintf (LOG_STREAM, "before LM application:\n");
      PrintTokSet (dec, ts);
      fprintf (LOG_STREAM, "++++++++++\n");
   }
   ...
   */
   ...
   if (trace & T_WORD) {
      fprintf (LOG_STREAM, "after LM application:\n");
      PrintTokSet (dec, ts);
      fprintf (LOG_STREAM, "++++++++++\n");
   }
}
                   ...
                   AdaptXForm *xform)
{
   ...
   if (trace & T_BEST) {
      fprintf (LOG_STREAM, "frame: %d beamLimit: %f\n", dec->frame, dec->beamLimit);
   }
   ...
   for (l = 0; l < dec->nLayers; ++l) {
      ...
      for (inst = dec->instsLayer[l]; inst; inst = inst->next) {
...
#ifdef DEBUG_TRACE
         if (trace & T_ACTIV) {
            fprintf (LOG_STREAM, "l %d active node %p ",l, inst);
            switch (inst->node->type) {
            case LN_MODEL:
               fprintf (LOG_STREAM, " HMM '%s' ",
                       FindMacroStruct (dec->net->hset, 'h', inst->node->data.hmm)->id->name);
               break;
            case LN_WORDEND:
               fprintf (LOG_STREAM, " WE '%s' ",
         ...
         /*         fprintf (LOG_STREAM, "BEST %p %f\n", inst->node, inst->best); */
      }
#if 0
   fprintf (LOG_STREAM, "MTS_copy: %d MTS_fast: %d  MTS slow: %d ", mts_copy, mts_fast, mts_slow);
   fprintf (LOG_STREAM, "MTS_newid: %d MTS_newidNTOK: %d\n", mts_newid, mts_newidNTOK);
#endif
      if (trace & T_TOKSTATS)
         fprintf (LOG_STREAM, "Pass1: %d active nodes in layer %d\n", nActive, l);
   }

   if (trace & T_TOKSTATS)
      fprintf (LOG_STREAM, "Sum Pass1: %d active models\n", modelActive);
   ...
   if (trace & T_BEST)
      fprintf (LOG_STREAM, "best token in HMM %p '%s' score %f %f\n", dec->bestInst->node, 
   ...
   for (l = 0; l < dec->nLayers; ++l) {
...
#ifdef DEBUG_TRACE
      if (trace & T_ACTIV)
         fprintf (LOG_STREAM, "external propagation for layer %d\n", l);
#endif
      nActive=0;
      ...
      if ((dec->weBeamWidth < dec->beamWidth) && 
         ...
         for (inst = dec->instsLayer[l]; inst; inst = next) {
            ...
            if (trace & T_PRUNE) {
               ...
               fprintf (LOG_STREAM, "pruning word end node %p '%s' score %f off beam limit %f\n", inst->node, name, inst->best, beamLimit);
      ...
      */
         ...
         if (inst->node->type != LN_WORDEND && inst->node->lmlaIdx != 0 &&
            ...
            if (trace & T_PRUNE) {
               ...
               if (inst->node->type == LN_MODEL)
                  ...
                  name = "NULL_NODE";
               fprintf (LOG_STREAM, "before lmla, node %p '%s' score %f\n", inst->node, name, inst->ts[0].score);
            }
#endif
            ...
            if (inst->ts->n > 0) {      /* UpLMLA might have killed the entire TS, esp. in latlm */
               ...
               if (trace & T_PRUNE) {
                  ...
                  if (inst->ts[0].score > inst->best) 
                     fprintf (LOG_STREAM, "after lmla, node %p '%s' score %f\n", inst->node, name, inst->ts[0].score);
               }
#endif
               best = inst->ts[0].score;
               if (best > inst->best)
               ...
               if (trace & T_PRUNE) {
                  ...
                  if (inst->node->type == LN_MODEL)
                     ...
                     name = "NULL_NODE";
                  fprintf (LOG_STREAM, "after lmla, node %p '%s' has no active token : %d\n", inst->node, name, inst->ts->n);
               }
#endif
            }
         }
         ...
         if (inst->best < beamLimit) {
            ...
            if (trace & T_PRUNE) {
               ...
               if (inst->node->type == LN_MODEL)
                  ...
                  name = "NULL_NODE";
               fprintf (LOG_STREAM, "pruning node %p '%s' score %f off beam limit %f\n", inst->node, name, inst->best, beamLimit);
      ...
      fprintf (LOG_STREAM, "MTS_copy: %d MTS_fast: %d  MTS slow: %d ", mts_copy, mts_fast, mts_slow);
      fprintf (LOG_STREAM, "MTS_newid: %d MTS_newidNTOK: %d\n", mts_newid, mts_newidNTOK);
      fprintf (LOG_STREAM, "LMCacheLA:  %d hits  %d misses\n", 
              dec->lmCache->laHit, dec->lmCache->laMiss);
#endif
      if (trace & T_TOKSTATS)
         fprintf (LOG_STREAM, "Pass2: %d active nodes in layer %d\n", nActive, l);

   }    /* for layer */

   if (trace & T_TOKSTATS)
      fprintf (LOG_STREAM, "Sum Pass2: %d %f \n", modelActive, dec->curBeamWidth);
...
#define MMP_NBINS 128
   if (dec->maxModel > 0) {
      ...
      if (nhist > dec->maxModel) {
         ...
         if (trace & T_PRUNE)
            fprintf (LOG_STREAM, "nMod: %d, dec->maxModel: %d\n", nMod, dec->maxModel);

         if (trace & T_PRUNE)
            fprintf (LOG_STREAM, "beam old: %f  ", dec->curBeamWidth);
         dec->curBeamWidth = (binWidth * i > maxLNBeamFlr * dec->beamWidth) ? binWidth * i : maxLNBeamFlr * dec->beamWidth;
         if (trace & T_PRUNE)
            fprintf (LOG_STREAM, "  new: %f\n", dec->curBeamWidth);
      }
...
#if 0
   fprintf (LOG_STREAM, "cacheHits: %d  cacheMisses: %d\n", 
           dec->outPCache->cacheHit, dec->outPCache->cacheMiss);
   fprintf (LOG_STREAM, "MTS_copy: %d MTS_fast: %d  MTS slow: %d ", mts_copy, mts_fast, mts_slow);
   fprintf (LOG_STREAM, "MTS_newid: %d MTS_newidNTOK: %d\n", mts_newid, mts_newidNTOK);
   fprintf (LOG_STREAM, "tokSetIDcount: %d\n", dec->tokSetIdCount);
   fprintf (LOG_STREAM, "PI_LR: %d  PI_GEN: %d\n", PI_LR, PI_GEN);
#endif
   PI_LR = PI_GEN = 0;
   dec->outPCache->cacheHit = dec->outPCache->cacheMiss = 0;

#if 0
   fprintf (LOG_STREAM, "LMCacheTrans:  %d hits  %d misses\n", 
