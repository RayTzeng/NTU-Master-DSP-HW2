...
#define HASH1(p1,s)  (((intptr_t)(p1))%(s))
...
void CollectPhoneStats (MemHeap *heap, TLexNet *net)
{
   ...
   */
   if (trace & T_NETCON)
      fprintf (LOG_STREAM, "adding extra nodes for single phone words:\n");

   for (i = 1; i <= net->nlexP; ++i) {
      if (trace & T_NETCON)
         fprintf (LOG_STREAM, "P='%s'  ", net->lexP[i]->name);
      p = net->lexP[i];
      for (j = 1; j <= net->nlexZ; ++j) {
         if (trace & T_NETCON)
            fprintf (LOG_STREAM, "z='%s' ", net->lexZ[j]->name);
         z = net->lexZ[j];
   ...
   if (trace & T_NETCON) { 
      ...
      fprintf (LOG_STREAM, "nlexA = %d   ", net->nlexA);
      for (i = 1; i <= net->nlexA; ++i)
         fprintf (LOG_STREAM, "%s ", net->lexA[i]->name);
      fprintf (LOG_STREAM, "\n");
      
      fprintf (LOG_STREAM, "nlexZ = %d   ", net->nlexZ);
      for (i = 1; i <= net->nlexZ; ++i)
         fprintf (LOG_STREAM, "%s ", net->lexZ[i]->name);
      fprintf (LOG_STREAM, "\n");
      
      fprintf (LOG_STREAM, "nlexAB = %d   ", net->nlexAB);
      for (i = 0; i < LEX_CON_HASH_SIZE; ++i)
         for (lcn = net->lexABhash[i]; lcn; lcn = lcn->next)
            fprintf (LOG_STREAM, "%s-%s  ", lcn->lc->name, lcn->rc->name);
      fprintf (LOG_STREAM, "\n");
      
      fprintf (LOG_STREAM, "nlexYZ = %d   ", net->nlexYZ);
      for (i = 0; i < LEX_CON_HASH_SIZE; ++i)
...
{     
   ...
   if (trace & T_NETCON) {
      TLexNode *lcn;
      fprintf (LOG_STREAM, "nlexSA = %d   ", net->nlexSA);
      for (i = 0; i < LEX_CON_HASH_SIZE; ++i)
...
void CreateZnodes (MemHeap *heap, TLexNet *net)
{
   ...
   if (trace & T_NETCON) {
      TLexNode *lcn;
      fprintf (LOG_STREAM, "nlexZS = %d   ", net->nlexZS);
      for (i = 0; i < LEX_CON_HASH_SIZE; ++i)
   ...
   if (trace & T_NETCON) {
      fprintf (LOG_STREAM, "nNodeZ = %d   ", net->nNodeZ);
      for (i = 0; i < LEX_MOD_HASH_SIZE; ++i)
         for (lnYZA = net->nodeZhash[i]; lnYZA; lnYZA = lnYZA->next)
            fprintf (LOG_STREAM, "yza_hmm %p nlinks %d\n", lnYZA->hmm, lnYZA->nlinks);
   }
}
...
void CreateSILnodes (MemHeap *heap, TLexNet *net)
{
   ...
   for (i = 0; i < LEX_CON_HASH_SIZE; ++i)
      for (lcnZS = net->lexZShash[i]; lcnZS; lcnZS = lcnZS->next) {
         z = lcnZS->lc;
         s = lcnZS->rc;
         /*         fprintf (LOG_STREAM, "ZS node %p %s-%s i %d\n", lcnZS, z->name, s->name, i); */
         ...
         if (s == sil) {        /* sil node */
            ...
            for (j = 1; j <= net->nlexA; ++j) {
               ...
               if (a != sil) {          /* list of A contxts includes sil! */
                  ...
                  /*  fprintf (LOG_STREAM, "  sil SA node %s-%s\n", lcnSA->lc->name, lcnSA->rc->name); */
            ...
            /*  fprintf (LOG_STREAM, "  sp  SA node %s-%s\n", lcnSA->lc->name, lcnSA->rc->name); */
            AddLink (heap, ln, lcnSA);
         }
      }
}
...
void Handle1PhonePron (MemHeap *heap, TLexNet *net, Pron pron)
{
   ...
   if (trace & T_NETCON)
      fprintf (LOG_STREAM, "handle single phone pron for '%s'\n", pron->word->wordName->name);
   ...
   pron->aux = (Ptr) (intptr_t) (int) pronid;
   ...
   */
   for (i = 1; i <= net->nlexZ; ++i) {
      if (trace & T_NETCON)
         fprintf (LOG_STREAM, "Z node '%s' P node '%s'\n", net->lexZ[i]->name, p->name);
      z = net->lexZ[i];
            ...
            for (pron = word->pron; pron ; pron = pron->next) {
               if (pron->aux == (Ptr) 1) {
                  if (pron->nphones >= 2) {
                     ...
                     /*  fprintf (LOG_STREAM, "one- or two-phone word (%s) ignored\n", word->wordName->name); */
                  Handle1PhonePron (heap, net, pron);
                  }
               }
            }
   if (trace & T_NETCON)
      fprintf (LOG_STREAM, "nodes shared in prefix tree: %d\n", nshared);
}
...
void CreateStartEnd (MemHeap *heap, TLexNet *tnet)
{
   ...
   lnWe->pron->aux = (Ptr) (intptr_t) lnWe->loWE;
...
int TraverseTree (TLexNode *ln, int start, int *lmlaCount)
{
   ...
   if (ln->type == LN_WORDEND) {
      ...
      ln->pron->aux = (Ptr) (intptr_t) ln->loWE;
   ...
   */
   ...
   for (i = 0; i < LEX_CON_HASH_SIZE; ++i)
      for (ln = tnet->lexABhash[i]; ln; ln = ln->next) {
         ...
         if (trace & T_NETCON)
            fprintf (LOG_STREAM, "AB %s-%s  loWE %d hiWE %d  lmlaIdx %d\n", ln->lc->name, ln->rc->name, 
   ...
   fprintf (LOG_STREAM, "lmla count %d\n", tnet->lmlaCount );
   fprintf (LOG_STREAM, "nprons %d\n", highest);
...
LexNet *ConvertTLex2Lex (MemHeap *heap, TLexNet *tnet)
{
   ...
   if (trace &T_NET) {
      fprintf (LOG_STREAM, "number of nodes:\n");
      fprintf (LOG_STREAM, " A model nodes: %d\n", tnet->nNodeA);
      fprintf (LOG_STREAM, " AB context nodes: %d\n", tnet->nlexAB);
      fprintf (LOG_STREAM, " B-Y model nodes: %d\n", tnet->nNodeBY);
      fprintf (LOG_STREAM, " YZ context nodes: %d\n", tnet->nlexYZ);
      fprintf (LOG_STREAM, " Z model nodes: %d\n", tnet->nNodeZ);
      fprintf (LOG_STREAM, " ZS context nodes: %d\n", tnet->nlexZS);
      fprintf (LOG_STREAM, " SIL model nodes: %d\n", tnet->nNodeSIL);
      fprintf (LOG_STREAM, " SA context nodes: %d\n", tnet->nlexSA);
      fprintf (LOG_STREAM, "total: %d\n", tnet->nNodeA + tnet->nlexAB + tnet->nNodeBY + tnet->nlexYZ +
   ...
   for (i = 0; i < NLAYERS; ++i) {
      ...
      if (trace & T_NET)
         fprintf (LOG_STREAM, "layer %d contains %d nodes\n", i, tnet->nNodesLayer[i]);
   }
   if (trace & T_NET)
      fprintf (LOG_STREAM, "total: %d nodes\n", nn);
   ...
   for (tln = tnet->root; tln; tln = tln->chain) {
      ...
      /*       fprintf (LOG_STREAM, "LMLA idx %d lo %d hi %d\n", tln->lmlaIdx, tln->loWE, tln->hiWE); */
   ...
   if (trace & T_NET)
      fprintf (LOG_STREAM, "converted %d links\n", nlTotal);
                      ...
                      char *startWord, char *endWord, Boolean silDict)
{
   ...
   for (i = 0; i < MACHASHSIZE; i++)
	 ...
	 /* 	 if (ml->type == 'l') */
	 fprintf (LOG_STREAM, "macro type: %c  name: %s  ptr: %x nUse: %d\n", 
...
#if 0           /* debug code: print contents of A layer */
   {
      ...
      fprintf (LOG_STREAM, "nNodeA = %d   \n", tnet->nNodeA);
      for (i = 0; i < LEX_MOD_HASH_SIZE; ++i)
         for (ln = tnet->nodeAhash[i]; ln; ln = ln->next) {
            fprintf (LOG_STREAM, "a_hmm %s nlinks %d ", FindMacroStruct (tnet->hset, 'h', ln->hmm)->id->name,
                    ln->nlinks);
            for (ll = ln->link; ll; ll = ll->next) {
               fprintf (LOG_STREAM, " %s-%s ", ll->end->lc->name, ll->end->rc->name);
            }
            fprintf (LOG_STREAM, "\n");
         }
...
#if 0
   fprintf (LOG_STREAM, "nlexA   %6d\n", tnet->nlexA);
   fprintf (LOG_STREAM, "nlexZ   %6d\n\n", tnet->nlexZ);

   fprintf (LOG_STREAM, "Nodes:\n");
   fprintf (LOG_STREAM, "nNodeA   %6d\n", tnet->nNodeA);
   fprintf (LOG_STREAM, "nlexAB   %6d\n", tnet->nlexAB);
   fprintf (LOG_STREAM, "nNodeBY  %6d\n", tnet->nNodeBY);
   fprintf (LOG_STREAM, "nlexYZ   %6d\n", tnet->nlexYZ);
   fprintf (LOG_STREAM, "nNodeZ   %6d\n", tnet->nNodeZ);
   fprintf (LOG_STREAM, "nlexZS   %6d\n", tnet->nlexZS);
   fprintf (LOG_STREAM, "nlexSA   %6d\n", tnet->nlexSA);
   fprintf (LOG_STREAM, "total    %6d\n", tnet->nNodeA + tnet->nlexAB + tnet->nNodeBY + 
   ...
   net->startPron = (PronId) (intptr_t) GetWord (voc, tnet->startId, FALSE)->pron->aux;
   net->endPron = (PronId) (intptr_t) GetWord (voc, tnet->endId, FALSE)->pron->aux;
         ...
         for (forw = n->link[DIR_FORW]; forw; forw = forw->next[DIR_FORW]) {
            ...
            if (left->data.monoId && right->data.monoId) {
               ...
               for (i = 2; i < hmm->numStates; ++i) {
                  fprintf (LOG_STREAM, " %4d ", hmm->svec[i].info->sIdx);
               }
               fprintf (LOG_STREAM, "\n");
            }
         }
